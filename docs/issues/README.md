...
| [05-concurrency-and-performance.md](05-concurrency-and-performance.md) | rayon, cancellation, test fixtures | 3 |
...
Thirty-six entries: **2 critical, 9 serious, 14 moderate, 11 friction**.
...
| Running against this machine rather than a fixture | 6 | Every one was a wrong *number* |
| Reasoning about the code while changing something nearby | 6 | Includes the helper's read hole and one near-miss never shipped |
...
**Running against this machine** found six defects, all of them wrong figures, and a fixture could not
have found any of them — a fixture contains what its author expected, so if the expectation is wrong
the fixture is wrong in the same direction and the test passes. dpkg's real database and this machine's
real snapd output both disagreed with what I expected.
...
not caught at all. That asymmetry is the argument for [09-patterns.md §7](09-patterns.md).
